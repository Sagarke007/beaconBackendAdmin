...
async def get_user_client_projects(
    ...
    try:
        ...
        if not user_id:
            return HTTPResponse.failed(status_code=400, detail="User ID not found in token")

        # Get the client and projects for this user
        client_id = manager.get_client_for_user(user_id)
        project_ids = manager.get_projects_for_user(user_id)

        if not client_id:
            HTTPResponse().success(response_message= "No client found for this user")
        ...
        return HTTPResponse().failed(
            status_code=500,
            detail=f"Failed to retrieve client and projects: {str(e)}"
        )